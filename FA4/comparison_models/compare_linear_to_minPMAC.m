...
dist_range = [1,8]; % have to generate random float value between 1m and 5m
...
while idx <= num_samples
    try
        ...
        % loading Quadriga channel file
        filename = sprintf('channel_files/Quadriga_WiFi/Tx3Rx2.mat');
        load(filename,"H");
        fprintf ("channel file loaded");
        ...
        % data_samples(idx).FEAS_FLA2 = FEAS_FLAG2;
        % data_samples(idx).bu_a2 = bu_a2 ;
        % data_samples(idx).info2 = info2;
        % data_samples(idx).bu_a_lin = bu_a_lin ;
        % data_samples(idx).bsum_lin = bsum_lin ;
        % data_samples(idx).bsum = bsum ;
        % data_samples(idx).bun_lin = bun_lin ;
        % data_samples(idx).transmission_power_db = 10*log10(transmission_power(idx));
