function [H] = get_channel(U, Ly, hx, hy, fc, BW, fft_length, scenario, manual_fading, transmission_type, verbose)
    ...
    for antenna = 1:Ly
        ...
        for i = 1:numel(c)
           ...
           if manual_fading == 1
               ...
               h(r_idx, antenna, :) = c(i).fr(BW, fft_length) * 10^(-fad_db/20); 
           else
               h(r_idx, antenna, :) = c(i).fr(BW, fft_length);
           end
        end
    end
