...
%                                     ones(1, Lx), cb);
        toc 
         ...
         E_GDFE(i) = sum(Eun(3,:), "all");
         b_GDFE(i) = sum(bun(3,:), "all");
         E_lin(i)  = sum(Rxx, "all") ;
         b_lin(i)  = bu_a_lin(3);        
...
disp('Done')
%%
filename = sprintf('wifi_results/fig_3/fig_3_data.mat');
...
plot(fft_length, nanmean(BB_GDFE, 1) ./fft_length,  'o-', 'DisplayName', 'Proposed Algorithm', 'LineWidth', 2)
...
ylabel('Data Rate Sum per tone (bit/s/Hz)', 'fontsize', 18, 'interpreter', 'latex')
legend('Location', 'southeast', 'fontsize', 13, 'interpreter', 'latex')
set(f, 'Units', 'Inches', 'Position', [0, 0, 8, 6]);
print(f, sprintf('wifi_results/fig_3/fig3_plot.pdf'), '-dpdf', '-bestfit')
